...
def convert_and_upload_supervisely_project(
    ...
    bboxes_path = os.path.join("EALPR-master","EALPR- LP characters dataset","Characters Labeling")
    images_path = os.path.join("EALPR-master","EALPR- Plates dataset")
    classes_path = os.path.join(bboxes_path, "classes.txt")
    bbox_ext = ".txt"
    ds_name = "ds"
    batch_size = 30
    bad_data = ['EALPR-master/EALPR- Plates dataset/1088_license_plate_1.png']
            ...
            with open(bbox_path) as f:
                ...
                for curr_data in content:
                    if len(curr_data) != 0:
                        ...
                        obj_class = id2class[curr_data[0]]
    ...
    with open(classes_path) as f:
                content = f.read().split("\n")
                for id, curr_data in enumerate(content):
                    id2class[id] = sly.ObjClass(curr_data, sly.Rectangle)


    project = api.project.create(workspace_id, project_name, change_name_if_conflict=True)
    meta = sly.ProjectMeta(obj_classes=list(id2class.values()))
    api.project.update_meta(project.id, meta.to_json())

    dataset = api.dataset.create(project.id, ds_name, change_name_if_conflict=True)

    all_images = [im_name for im_name in os.listdir(images_path) if im_name != "2018.jpg"]
    all_images = [im_name for im_name in all_images if im_name.lower().endswith(".png")]
    ...
    for img_names_batch in sly.batched(all_images, batch_size=batch_size):
        ...
        try:
            ...
            for name, path in zip(img_names_batch, img_pathes_batch):
                try:
                    info = api.image.upload_path(dataset.id, name, path)
